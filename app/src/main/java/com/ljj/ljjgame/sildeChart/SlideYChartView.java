...
public class SlideYChartView extends View {
    ...
    private int hLineCount = 11;// 横线数量
    private int vLineCount = 12;// 竖线数量
    ...
    private void init() {
        ...
        if (data.size() == 0) {
            ...
            for (int i = 0; i < 12; i ++) {
                SlideYChartPoint point = new SlideYChartPoint(i*2, 10);
                line.addPoint(point);
            }
            data.add(line);
...
//        tv.setText("x:" + movePoint.getX() + ",y:" + format2Bit(movePoint.getY()));
        int time = (int) movePoint.getX();
        int lightness = (int) movePoint.getY();
        tv.setText("时间:" + time + ",亮度:" + lightness);
