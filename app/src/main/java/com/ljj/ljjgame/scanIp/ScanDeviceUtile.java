...
public class ScanDeviceUtile {
    ...
    private List<String> hostNameList = new ArrayList<String>();// ping成功的IP地址
    private ThreadPoolExecutor mExecutors;// 线程池对象
...
//        private ThreadPoolExecutor mExecutor;// 线程池对象
        ...
        ThreadPoolExecutor mExecutor = new ThreadPoolExecutor(CORE_POOL_SIZE, MAX_IMUM_POOL_SIZE,
...
//        for (int i = 1; i < 255; i++) {// 创建256个线程分别去ping
        ...
        for (int i = ss; i < sz; i++) {// 创建256个线程分别去ping
            ...
            Runnable run = new Runnable() {
                ...
                public void run() {
                    ...
                    try {
                       ...
                       LogUtil.e(TAG+ "正在扫描的IP地址为：" + currnetIp + "返回值为：" + result);
                        if (result == 0) {
//                           LogUtil.e(TAG+ "扫描成功,Ip地址为：" + currnetIp);
                            mIpList.add(currnetIp);

//                            InetAddress host = InetAddress.getByName(currnetIp);
                            NbtAddress host = NbtAddress.getByName(currnetIp);
                            String hostName=host.firstCalledName();
                            String hostNames=host.nextCalledName();
                            hostNameList.add(hostNames);
//                            DeviceInfo deviceInfo=new DeviceInfo(hostName, ip);
//                            devices.add(deviceInfo);
//                            LogUtil.e(hostName+":"+currnetIp);
//                            LogUtil.e(hostNames+":"+currnetIp);
                        } else {
                            // 扫描失败
//                           LogUtil.e(TAG, "扫描失败");
                        }
                    } catch (Exception e) {
                        LogUtil.e(TAG + "扫描异常" + e.toString());
                    } finally {
                        if (mProcess != null) {
                            mProcess.destroy();
                        }
        ...
        while (true) {
            try {
                if (mExecutor.isTerminated()) {// 扫描结束,开始验证
                    LogUtil.e(TAG + "扫描结束,总共成功扫描到" + mIpList.size() + "个设备.");
                    LogUtil.e("设备列表：" + new Gson().toJson(mIpList));
                    LogUtil.e("hostNameList：" + new Gson().toJson(hostNameList));
//                    if (times <4) {
//                        times++;
//
//                        scan();
...
//                    }
    ...
    public void getHostName() {
        mExecutors = new ThreadPoolExecutor(CORE_POOL_SIZE, MAX_IMUM_POOL_SIZE,
                2000, TimeUnit.MILLISECONDS, new ArrayBlockingQueue<Runnable>(
                CORE_POOL_SIZE));
        ...
        for (int j = 0; j < mIpList.size(); j++) {
            // 创建256个线程分别去ping
            final int js = j;// 存放ip最后一位地址 1-255
...
//            int finalJ = j;
            Runnable runs = new Runnable() {
                ...
                @Override
                public void run() {
                    NbtAddress host = null;
                    try {
                        host = NbtAddress.getByName(mIpList.get(js));
        ...
        while (true) {
            try {
                if (mExecutors.isTerminated()) {// 扫描结束,开始验证
                    LogUtil.e(TAG + "扫描结束,总共成功扫描到" + mIpList.size() + "个设备.");
                    LogUtil.e("设备列表：" + new Gson().toJson(mIpList));
                    LogUtil.e("hostNameList：" + new Gson().toJson(hostNameList));
