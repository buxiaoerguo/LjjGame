...
//线的颜色
    ...
    private void obtainAttribute(Context context, AttributeSet attrs) {
        ...
        try {
            linePaintColor = typedArray.getColor(R.styleable.BrokenLineView_linePaintColor, Color.BLUE);

            brokenLinePaintColor = typedArray.getColor(R.styleable.BrokenLineView_brokenLinePaintColor, Color.BLACK);

            textPaintColor = typedArray.getColor(R.styleable.BrokenLineView_textPaintColor, Color.BLACK);
            textPaintSize = typedArray.getDimensionPixelSize(R.styleable.BrokenLineView_textPaintSize, sp2px(context, 10));

            circlePaintColor = typedArray.getColor(R.styleable.BrokenLineView_circlePaintColor, Color.BLACK);
            circleRadius = typedArray.getDimensionPixelSize(R.styleable.BrokenLineView_circleRadius, dp2px(context, 3));

            textVerticesPaintColor = typedArray.getColor(R.styleable.BrokenLineView_textVerticesPaintColor, Color.BLACK);
            textVerticesPaintSize = typedArray.getDimensionPixelSize(R.styleable.BrokenLineView_textVerticesPaintSize, sp2px(context, 10));
            lineWidth = typedArray.getDimensionPixelSize(R.styleable.BrokenLineView_lineWidth, dp2px(context, 1));
        } finally {
            typedArray.recycle();
        }
    }
...
//每一部分y轴的取值
    ...
    private void drawBrokenLine(Canvas canvas) {
        ...
        for (int i = 0; i < brokenData.length; i++) {
            ...
            int sts= (int) brokenData[i];
            canvas.drawText(String.valueOf(sts), startX + singleWidth * i + spacing, startY - singleHeight * (brokenData[i] - minY) / singleY - spacing, mTextVerticesPaint);
//            canvas.drawText();
        }
